...
- DeepSeek 对话：Web 开发模式通过本地 Express 服务转发请求，Windows 桌面版通过 Tauri 内置命令直接调用模型。
...
- Windows 桌面正式版：应用内保存 API Key、Base URL 和模型名到当前用户配置文件，启动后可直接使用。

## 后续升级重点

`v0.6` 的标志性升级已经完成：桌面版不再要求用户手动启动本地 Express 服务，也不需要编辑项目 `.env`。Windows 用户启动桌面应用后，可以在应用内填写 API Key、Base URL 和模型名，配置会保存到当前用户的本机配置文件。

1. **桌面发布打磨**
   - 补齐 GitHub Release 流程和安装包发布说明。
   - 增加安装包签名与图标资源。
   - 增加桌面端自动更新能力。

2. **配置安全**
   - 当前配置文件默认不加密，后续可升级为系统钥匙串或加密配置文件。
   - 增加配置导入/导出，但避免把 API Key 混进访谈备份。
   - 增加更明显的“不要共享配置文件”提示。
...
5. **工程质量**
   - 继续拆分 `src/App.tsx`：把存储、导出、报告解析、会话筛选等逻辑移到独立模块。
...
适合想像普通桌面软件一样使用的人。当前仓库已经支持构建 Windows 安装包；如果 GitHub Release 页面提供安装包，可以直接下载安装。
...
点击“保存并测试连接”后，配置会写入当前 Windows 用户目录：

```text
%APPDATA%\Chat2Yourself\config.json
```

这个文件包含你的 API Key，请不要提交、截图或分享它。访谈记录仍保存在应用前端的本地存储中，模型配置不会写入 `localStorage`。
...
`npm run desktop:build` 生成的桌面包会使用 Tauri 内置模型命令，不需要用户另外运行 `npm run dev`。
...
- `.env` 已被 Git 忽略，Web 开发模式的 API Key 不应提交到仓库。
- Windows 桌面版会把模型配置保存到 `%APPDATA%\Chat2Yourself\config.json`，这个文件包含 API Key，请不要共享。
- 会话记录默认保存在当前浏览器的 `localStorage` 中。
- 不同浏览器、不同设备之间不会自动同步数据。
- 发送消息或生成画像时，相关会话内容会发送给你配置的 DeepSeek API；Web 开发模式经本地 Express 服务转发，桌面版经 Tauri 内置命令发送。
...
### Web 模式提示 Missing DEEPSEEK_API_KEY
...
在桌面配置页填写 API Key、Base URL 和 Model，然后点击“保存并测试连接”。如果测试通过，应用会把配置保存到 `%APPDATA%\Chat2Yourself\config.json`。

### 桌面版想换 API Key 或模型

在侧栏“本地连接”里点击“修改配置”，重新填写并保存。也可以点击“清除配置”删除本机配置文件。
...
- 桌面配置文件默认不加密，里面包含 API Key。
- 本地访谈数据没有加密，备份文件也没有密码保护。
- 没有账号体系、云同步或多人共享能力。
